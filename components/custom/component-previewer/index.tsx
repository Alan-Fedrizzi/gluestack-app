    ...
    if (propConfig.control.type === 'boolean') {
      return (
        <Switch
          ...
          value={selectedValues[key] as boolean}
          onToggle={() => handleChange(key, !(selectedValues[key] as boolean))}
        />
      );
    }
        ...
        >
          ...
          >
            <SelectInput
              ...
              placeholder={(selectedValues[key] as string) ?? ''}
            />
            <SelectIcon
              size="xl"
      ...
      <Box className="md:flex-row">
        <Center
          className={`min-h-[100px] ${
            Object.keys(props).length > 0 ? 'md:border-r' : ''
          } border-outline-100 flex-1 py-5 w-full`}
        >
